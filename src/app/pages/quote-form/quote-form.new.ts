...
export class QuoteForm implements OnInit {
  ...
  public filteredMakes: Array<{id: string, name: string}> = [];
  ...
  ngOnInit(): void {
    ...
    // Subscribe to vehicle type changes to filter makes
  ...
  filterMakesByType(type: string): void {
    if (!type) {
      this.filteredMakes = [];
      return;
    }

    // Map of vehicle types to compatible makes
    const makesByType: { [key: string]: string[] } = {
      'sedan': ['toyota', 'honda', 'ford', 'chevrolet'],
      'suv': ['toyota', 'honda', 'ford', 'chevrolet'],
      'truck': ['ford', 'chevrolet'],
      'van': ['toyota', 'honda']
    };

    const compatibleMakeIds = makesByType[type] || [];
    this.filteredMakes = this.vehicleMakes
      .filter(make => compatibleMakeIds.includes(make.id))
